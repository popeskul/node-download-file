...
function saveCSVCars(name, arr) {
  try {
    ...
    const fileTitle = `${name}-${generateDate(Date.now())}.csv`;
    fs.writeFileSync(fileTitle, arr);
    return fileTitle;
  } catch (error) {
    console.log(error);
  }
...
function render(req, res) {
  if (req.url === '/') {
    ...
    res.write('<a href="/tesla" id="tesla">обновить данные по Тесле</a>');
    res.end();
    ...
    request(
      ...
      (e, r, body) => {
        ...
        downloadLink = saveCSVCars('pasha', CARS_STRING);

        res.write(generateCSVTable(CARS_STRING));
        res.write(`<a href="/${downloadLink}" download>${downloadLink}</a>`);
        res.end();
      }
    );
  } else if (req.url.indexOf('.csv') !== -1) {
    res.setHeader('Content-Type', 'attachment/csv');
    fs.readFile(path.join(__dirname, `/${downloadLink}`), 'utf-8', (e) =>
      console.log('error', e)
    );
